#!/usr/bin/env python3
"""
Stage 1 Case 2 (v2) — Corrected methodology.

Fixes from Case 1:
  1. assistant message = summary text + tool_use block (not the report body itself)
  2. baseline vs treatment differ ONLY in tool_result content (full body vs stripped marker)
  3. subagent instructions explicitly ban tool usage

Produces two prompt files:
  - case2_prompt_full.txt      = filler + proper SAGOL turn structure (tool_result = full body)
  - case2_prompt_stripped.txt   = filler + proper SAGOL turn structure (tool_result = stripped marker)
"""
...
FILLER_CHAR_TARGET = 150_000


# The user request that triggers the report
SYNTH_USER_REQUEST = (
    "잠깐 다른 걸 부탁하자. 우리 포털 auth 시스템 아키텍처 리뷰를 "
    "상세하게 써줘 — 현재 구조, 취약점, 개선안 포함해서."
)

# The assistant's VISIBLE summary (what stays in context in both conditions)
ASSISTANT_SUMMARY = (
    "auth 시스템 리뷰를 작성했습니다. "
    "4개 레이어(Edge Nginx+Lua / Auth Gateway Spring Cloud / Identity Service Spring Boot / Redis Session Cluster)를 분석하고, "
    "6개 취약점(V-01 refresh token race condition ~ V-06 CORS wildcard)과 P0/P1/P2 개선안을 정리했습니다."
)

# The full report body (goes into tool input AND baseline tool_result)
REPORT_BODY = """# EZ-PLANET Portal Auth System — Architecture Review
...
# The stripped marker (what SAGOL MCP server actually returns)
STRIPPED_MARKER = (
    "[report:1776230000000-a1b2c3d4] EZ-PLANET Portal Auth System — Architecture Review\n"
    "(full body: .sagol/reports/1776230000000-a1b2c3d4.md)"
)

# The followup question (independent of report topic)
FOLLOWUP = (
...
# Subagent instruction header (shared between both conditions)
INSTRUCTION_HEADER = """당신은 Claude Code 세션 도중 유저와 대화 중입니다.
아래는 이 세션의 대화 이력입니다. 마지막 유저 메시지에 응답하세요.

중요 규칙:
- 도구(Read, Write, Grep, Glob, Bash 등)를 절대 사용하지 마세요. 텍스트로만 응답하세요.
- 파일을 읽거나 검색하지 마세요. 대화 맥락과 일반 지식으로만 답하세요.
- 코드 예시가 필요하면 대화 내용 기반으로 추정해서 작성하세요.

대화 이력:
"""

# Synthetic section — shows the tool_use / tool_result structure
SYNTH_SECTION_TEMPLATE = """
=== USER (report request) ===
{user_request}

=== ASSISTANT (report response) ===
{summary}

[tool_use: mcp__sagol__write_report]
  input: {{title: "EZ-PLANET Portal Auth System — Architecture Review", body: "(3,232자 상세 리뷰)"}}

[tool_result: mcp__sagol__write_report]
{tool_result}

=== USER (followup — different topic) ===
{followup}
"""
...
def main():
    ...
    # --- Baseline: tool_result = full report body ---
    synth_full = SYNTH_SECTION_TEMPLATE.format(
        user_request=SYNTH_USER_REQUEST,
        summary=ASSISTANT_SUMMARY,
        tool_result=REPORT_BODY,
        followup=FOLLOWUP,
    )
    prompt_full = INSTRUCTION_HEADER + filler_rendered + synth_full

    # --- Treatment: tool_result = stripped marker ---
    synth_stripped = SYNTH_SECTION_TEMPLATE.format(
        user_request=SYNTH_USER_REQUEST,
        summary=ASSISTANT_SUMMARY,
        tool_result=STRIPPED_MARKER,
        followup=FOLLOWUP,
    )
    prompt_stripped = INSTRUCTION_HEADER + filler_rendered + synth_stripped

    (OUT / "case2_prompt_full.txt").write_text(prompt_full)
    (OUT / "case2_prompt_stripped.txt").write_text(prompt_stripped)
    ...
    meta = {
        ...
        "prompt_full_chars": len(prompt_full),
        "prompt_stripped_chars": len(prompt_stripped),
        "char_reduction": len(prompt_full) - len(prompt_stripped),
        "reduction_pct": (len(prompt_full) - len(prompt_stripped))
        / max(len(prompt_full), 1)
        * 100,
        "report_body_chars": len(REPORT_BODY),
        "stripped_marker_chars": len(STRIPPED_MARKER),
        "assistant_summary_chars": len(ASSISTANT_SUMMARY),
        "followup_chars": len(FOLLOWUP),
        "methodology_fixes": [
            "assistant message = summary text (identical in both conditions)",
            "tool_use/tool_result structure present in both conditions",
            "only tool_result content differs (full body vs stripped marker)",
            "subagent instructed: no tool usage, text-only response",
        ],
    }
    (OUT / "case2_meta.json").write_text(json.dumps(meta, indent=2, ensure_ascii=False))

    print()
    print("=== Case 2 prompts built ===")
